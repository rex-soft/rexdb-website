...
<p><code>DB.update(String sql, Object[] parameterArray)</code>方法用于执行一个带有预编译参数的插入/更新/删除SQL。其中，<code>parameterArray</code>参数是一个数组，数组中的元素按照顺序对应SQL语句中的<code>?</code>标记。Rexdb将按照顺序从数组中取值，并调用JDBC相关接口赋值，然后执行SQL。</p>
...
<p>除<code>数组</code>可以作为执行SQL的参数外，Rexdb还内置了一个类<code>org.rex.db.Ps</code>，它拥有丰富的操作接口，可以用于封装预编译参数。它可以指定字段类型、按照下标赋值，还可以为存储过程调用声明输出、输入输出参数，您可以根据实际情况选用。除此之外，Rexdb还支持<code>Map</code>和自定义的<code>实体类</code>作为执行SQL的参数。</p>

<p>除数组外，各种类型的参数调用示例如下：</p>

<p>1）使用内置的<code>org.rex.db.Ps</code>类作为预编译参数：</p>
...
<p>2）使用<code>Map</code>作为预编译参数：</p>
...
<p>3）使用实体类作为预编译参数。首先需要编写表<code>REX_TEST</code>对应的实体类：</p>
...
<p>请注意，在使用<code>Map</code>、<code>实体类</code>作为预编译参数时，SQL语句中的预编译参数标记不再是JDBC标准的<code>?</code>，而是被<code>#{*参数名称*}</code>取代，Rexdb在执行时，会根据标记中的<code>参数名称</code>查找<code>Map</code>、<code>实体类</code>中对应的属性值。其中，数据库字段名称和Java对象参数名称对应规则如下所示：</p>
...
<p>还需要额外注意的是，在使用实体类作为预编译参数时，实体类必须满足如下条件，才能被Rexdb正常调用：</p>

<ul>
<li>类是可以访问的</li>
<li>可以使用无参的构造函数创建类实例（启用动态字节码选项时需要调用）</li>
<li>参数需要有标准的<code>getter方法</code>
</li>
</ul>
...
<p>编写类<strong>TestUpdateBatch</strong>，内容如下：</p>
...
<p>在上面的类中，以<code>org.rex.db.Ps</code>数组作为批量插入的参数，数组中的每个元素都代表一条记录。执行<code>DB.batchUpdate(String sql, Ps[] pss)</code>后，数据库将写入10条记录。</p>
...
<p><code>DB.getMapList(String sql)</code>方法用于执行查询SQL，并返回一个包含有查询结果的<code>List</code>，其中数据库列名将被转换为Java命名风格。</p>

<p>其中，<code>List</code>中的元素<code>org.rex.RMap</code>是Rexdb框架提供的封装类，它继承自<code>java.util.HashMap</code>，额外提供了Java类型的自动转换功能，您可以方便的从该类中直接获取各种Java类型的值，例如直接获取<code>int</code>类型、<code>String</code>类型，或者<code>java.util.Date</code>类型的值，而不需要自行编写类型转换代码。</p>
...
<p>接口<code>DB.getList(String sql, Class resultClass, int offset, int rows)</code>根据预设的<code>offset</code>和<code>rows</code>参数查询相应的数据库记录，<code>offset</code>参数表示行偏移，<code>rows</code>参数表示要查询的行数。Rexdb在执行查询时，会根据数据库类型自动选择相应的方言，再根据方言对SQL进行封装。例如，Mysql中，实际执行的SQL语句为：</p>
...
<p>当没有编写与结果集对应的实体类时，则可以直接查询出一个<code>RMap</code>对象：</p>
...
<p>如果希望直接获取某一行中的某个字段值时，可以直接从<code>RMap</code>中取值。例如查询某张表的总记录数，可以直接调用<code>RMap.getInt(String key)</code>接口，以获取<code>int</code>类型的值。这可以节省您的代码量。但是请确保能够查询出记录，以防产生空指针异常。例如：</p>
...
<p>接口<code>DB.beginTransaction()</code>用于开启一个事物，<code>DB.commit()</code>和<code>DB.rollback()</code>分别用于提交和回滚事物。Rexdb的事物是线程级别的，事物一旦开启，将在整个用户线程中有效，直到事物被提交或者回滚。</p>
...
<p>则可以使用Rexdb的<code>DB.call(String sql)</code>接口调用该存储过程：</p>
...
<p>则可以使用<code>数组</code>、<code>Map</code>、<code>org.rex.db.Ps</code>、实体类等作为输入参数。例如，以以<code>org.rex.db.Ps</code>对象做参数时：</p>
...
<p>当存储过程定义了输出参数时，则需要使用<code>org.rex.db.Ps</code>对象声明该参数。例如，Mysql中有如下存储过程：</p>
...
<p>在调用存储过程时，需要使用<code>org.rex.db.Ps</code>对象声明输出参数的位置和类型。存储过程在调用后，将返回一个<code>RMap</code>对象，在该对象中可以获取输出参数的值：</p>
...
<p>在返回的<code>RMap</code>对象中，输出参数的键为<code>out_参数序号</code>。为方便调用后取值，也可以在声明输出参数时设置一个别名，例如：</p>
...
<p>上面的配置文件定义了2个数据源，分别是Mysql和Oracle数据库。其中，Oracle数据源定义了<code>id="oracleDs"</code>的属性，在调用<code>org.rex.DB</code>的接口时，可以通过首个参数指定该数据源。未定义<code>id</code>属性的是Rexdb的默认数据源，一个应用中只能定义1个默认数据源。</p>
...
<p>全局配置文件的“<em><code>/configuration/dataSource</code></em>”节点<code>class="org.apache.commons.dbcp.BasicDataSource"</code>属性定义了数据源的实现类，其子节点<code>property</code>用于配置数据源需要的属性。Rexdb在初始化时，会首先创建数据源实例，然后调用其<code>setter方法</code>赋值。</p>
...
<li>动态字节码：Rexdb支持javassist的动态字节码功能，当运行环境中具有javassist的Jar包，并且启用了相关配置时，Rexdb将使用动态字节码方式读写Java对象，此时，调用<code>DB.get(...)</code>接口和<code>DB.getList(...)</code>等接口时，将会获得更高效的性能。</li>
</ul>
